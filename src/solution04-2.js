import {add, all, any, compose, filter, flat, fork, fst, id, join, len, loadSync, map, or, pipe, reduce, slice, split, trace, traceMsg, transpose} from './common.js'
  ...
  ({draws, boards}) => {
    ...
    for (const d of draws) {
      ...
      if (winner = getWinner(acc)) {
        ...
        acc = filter (b => !isWinner (b)) (acc)
      }
    }
  },
  traceMsg(_ => 'Losing draw and board'),
  traceMsg(({draw}) => draw),
